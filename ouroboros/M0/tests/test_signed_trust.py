"""M1.6 — signed checkpoints, trust-key lifecycle, epoch binding."""
import dataclasses
...
def test_rotation_advances_exactly_one_epoch_and_retires_old_key(authority):
    ...
    with pytest.raises(SignedTrustError, match="fresh key id"):
        store.rotate("k1", Ed25519PrivateKey.generate().public_key())
    with pytest.raises(SignedTrustError, match="new key material"):
        store.rotate("k2", k1.public_key())
...
def test_retired_key_is_historical_only(journal_path, authority):
    ...
    k1 = Ed25519PrivateKey.generate()
    store.rotate("k1", k1.public_key())
    # Not acceptable as a *current* checkpoint...
    with pytest.raises(SignedTrustError, match="RETIRED"):
        store.verify(old_cp)
    # ...but may validate its own historical epoch.
    assert store.verify(old_cp, historical=True).key_id == "k0"
    # A retired key cannot vouch for the new epoch, even historically.
...
def test_signed_read_checks_signature_before_anchor(journal_path, authority):
    ...
    journal = journal_with(journal_path)
    anchor = JournalTrustAnchor.capture(journal.records())
    cp = sign_checkpoint(Ed25519PrivateKey.generate(), "k0", 0, anchor)  # bad signature, good anchor
    with pytest.raises(SignedTrustError):
    ...
    # Now rewrite history: signature fine, anchor fails
