...
def capture_evidence(run:Run,results,required_gates):
    e=VerificationEvidence(run.id,run.generation,run.verification_epoch,tuple(results),tuple(required_gates),''); return VerificationEvidence(e.run_id,e.generation,e.epoch,e.results,e.required_gates,e.recompute_digest())
