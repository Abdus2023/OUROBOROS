...
class TrustTransition:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.operation, TransitionOperation):
            raise SignedTrustError("lifecycle operation must be a known transition operation")
    ...
    def signed_payload(self) -> dict[str, Any]:
        return {"schema": LIFECYCLE_SCHEMA, "algorithm": self.algorithm, "operation": self.operation.value, "signer_key_id": self.signer_key_id, "from_epoch": self.from_epoch, "to_epoch": self.to_epoch, "target_key_id": self.target_key_id, "target_public_key": self.target_public_key.hex() if self.target_public_key is not None else None}
    ...
    def from_record(cls, record: Any) -> "TrustTransition":
        ...
        expected = {"schema", "algorithm", "operation", "signer_key_id", "from_epoch", "to_epoch", "target_key_id", "target_public_key", "signature"}
...
def sign_revocation(private_key: Ed25519PrivateKey, store: TrustStore, target_key_id: str) -> TrustTransition:
    active = store.active
    ...
    draft = TrustTransition(TransitionOperation.REVOKE, active.key_id, store.epoch, store.epoch, target_key_id, None, b"\0")
    return TrustTransition(draft.operation, draft.signer_key_id, draft.from_epoch, draft.to_epoch, draft.target_key_id, draft.target_public_key, private_key.sign(draft.signed_bytes()))
...
def verify_transition(statement: TrustTransition, store: TrustStore) -> None:
    ...
    try:
        store.active.verifier().verify(bytes(statement.signature), statement.signed_bytes())
...
def apply_transition(statement: TrustTransition, store: TrustStore) -> None:
    """Verify first, then invoke the private trust-store mutation primitive."""
    verify_transition(statement, store)
    if statement.operation is TransitionOperation.ROTATE:
        assert statement.target_public_key is not None
        store._apply_rotation(statement.target_key_id, statement.target_public_key)
    else:
        store._apply_revocation(statement.target_key_id)
